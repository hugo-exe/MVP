...
public class PanelProfits extends JPanel {
    ...
    private void inserirProspeccao() {
        try {
            ...
            JOptionPane.showMessageDialog(this, "Prospecção salva", "Mensagem", JOptionPane.INFORMATION_MESSAGE);
            limparCampos();
    
            // Persistir os dados em um arquivo
            salvarProspeccao(prospeccao, "prospeccao.txt");
        } catch (NumberFormatException e) {
            System.out.println("Erro ao converter ID da prospecção para um número inteiro.");
        } catch (IOException e) {
            JOptionPane.showMessageDialog(this, "Erro ao salvar prospecção: " + e.getMessage(), "Erro", JOptionPane.ERROR_MESSAGE);
        }
    }
    
    private void salvarProspeccao(Prospeccao prospeccao, String arquivo) throws IOException {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(arquivo))) {
            writer.write("ID: " + prospeccao.getIdProspeccao());
            writer.newLine();
            writer.write("Descrição: " + prospeccao.getDescricao());
            writer.newLine();
            writer.write("Valor: " + prospeccao.getValor());
            writer.newLine();
            writer.write("Parcelas: " + prospeccao.getParcelas());
            writer.newLine();
            writer.newLine();
        } catch (IOException e) {
            throw new IOException("Erro ao salvar prospecção: " + e.getMessage());
        }
    }

    private void alterarProspeccao() {
        try {
            int id = Integer.parseInt(textFieldId.getText());
            String arquivo = "prospeccao.txt";
            ...
            if (prospeccao != null && prospeccao.getIdProspeccao() == id) {
                ...
                if (novaDescricao != null) {
                    prospeccao.setDescricao(novaDescricao);
                    salvarProspeccao(prospeccao, arquivo);
                    JOptionPane.showMessageDialog(this, "Prospecção alterada com sucesso!", "Sucesso", JOptionPane.INFORMATION_MESSAGE);
                }
            } else {
                JOptionPane.showMessageDialog(this, "Prospecção não encontrada.", "Erro", JOptionPane.ERROR_MESSAGE);
            }
        } catch (NumberFormatException e) {
            System.out.println("Erro ao converter ID da prospecção para um número inteiro.");
        } catch (IOException e) {
            JOptionPane.showMessageDialog(this, "Erro ao acessar o arquivo de prospecção: " + e.getMessage(), "Erro", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void deletarProspeccao() {
        try {
            ...
            if (prospeccao != null && prospeccao.getIdProspeccao() == id) {
                ...
                JOptionPane.showMessageDialog(this, "prospeccaa deletada", "Erro", JOptionPane.INFORMATION_MESSAGE);
                outputStream.close();
            }
        } catch (NumberFormatException e) {
            System.out.println("Erro ao converter ID da prospecção para um número inteiro.");
    ...
    private void verProspeccao() {
    	JOptionPane.showMessageDialog(this, "nao fucionando ainda", "Erro", JOptionPane.INFORMATION_MESSAGE);
    }
